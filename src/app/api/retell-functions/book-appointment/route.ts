...
export async function POST(request: Request) {
  try {
    ...
    const patientPhone = args.patient_phone as string | undefined;
    const date = args.date as string | undefined;
    const time = args.time as string | undefined;
    const appointmentType = (args.appointment_type as string) || "general consultation";
    ...
    if (!patientName || !patientPhone || !date || !time) {
      return retellResponse({
        ...
        message: "I need the patient's full name, phone number, preferred date, and time to book an appointment.",
        missing: {
    ...
    if (isNaN(startTime.getTime())) {
      return retellResponse({
        ...
        message: `Could not parse date "${date}" and time "${time}". Please provide a valid date and time.`,
      });
    }
    ...
    // Send SMS in background — don't let failures block the booking response
    try {
      await sendSms({
        to: patientPhone,
      ...
      // SMS failed (e.g. Twilio not verified yet) — continue
    }
    ...
    return retellResponse({ error: "book-appointment failed", message: msg });
  }
}
